...
public class MainActivity extends FlutterActivity implements MethodCallHandler, FlutterPlugin {
    private static final String TAG = "TTS flutter";
    ...
    boolean isLanguageInstalled(String language) {
        ...
        if (isLanguageAvailable(locale)) {
            ...
            if (voiceToCheck != null) {
                ...
                if (android.os.Build.VERSION.SDK_INT >= android.os.Build.VERSION_CODES.LOLLIPOP) {
                    features = voiceToCheck.getFeatures();
                }
...
//            Log.d(TAG, "getLanguages: %d" + 10);
//            Log.d(TAG, "getLanguages: %d" + Build.VERSION_CODES.M);
    ...
    private void synthesizeToFile(String text, String fileName) {
        ...
        int result = 0;
        if (android.os.Build.VERSION.SDK_INT >= android.os.Build.VERSION_CODES.LOLLIPOP) {
            result = tts.synthesizeToFile(text, bundle, file, SYNTHESIZE_TO_FILE_PREFIX + uuid);
        }
...
public class MainActivity extends FlutterActivity {
    private static final String CHANNEL = "printing";

    @Override
    public void configureFlutterEngine(@NonNull FlutterEngine flutterEngine) {
        super.configureFlutterEngine(flutterEngine);
        MethodChannel channel = new MethodChannel(flutterEngine.getDartExecutor().getBinaryMessenger(), CHANNEL);
        PrintingHandler handler = new PrintingHandler(this.getActivity(), channel);
